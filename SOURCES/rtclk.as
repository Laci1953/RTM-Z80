...
roundrobin:                     ;RoundRobin algorithm follows...
        ...
        jr      z,3f
        ld      bc,PRI_OFF      ;Last active task changed...
	add	hl,bc
        ld      a,(hl)          ;A=priority of the current active task
	or	a		;if zero (default task)
	jr	nz,1f
	inc	a		;set it to 1
1:
...
3:
        ...
        jr      nz,check
again:                          ;TicsCount=0, try to rotate active task list
IF	SIO_RING
	call	GetSIOChars
ENDIF
        ld      hl,_TasksH      ;HL=active tasks list header
        call    __RotateL       ;L=1 if round-robin move done, else 0
	jr	z,check		;if list empty or has 1 element, go check signals
				;CARRY=0
	ld	hl,(_TasksH)	;if default task is first in the list
	ld	bc,TCB_Default
	sbc	hl,bc
	jr	z,again		;...rotate again to avoid setting default task as running task
check:
	ld	a,(Signals)
