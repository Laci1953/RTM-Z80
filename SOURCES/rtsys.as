...
IF	SYSSTS
	GLOBAL	mul32, div32, TypeRemStack, Sum@
ENDIF
...
STACKW_OFF	equ	29	;StackWarning (after system status fields)
;
IF	CMD
...
IF	SYSSTS
	di
	ld	a,1			;enable sampling
	ld	(CountTicks),a
					;erase local stack
	ld	hl,TCB_Default + TICS_OFF + 2
	ld	b,def_sp - (TCB_Default + TICS_OFF + 2)
	xor	a
clr:
	ld	(hl),a
	inc	hl
...
DefaultLoop:
	...
	ei
	nop		
	jr	z,DefaultLoop		;if shutdown marker found...
	di				;DISABLE Interrupts	
...
;place the data block at a multiple of 100H
...
PS2Tbl:
;0x0-0x7
        defb 0,0f9h,0,0f5h,0f3h,0f1h,0f2h,0fch
;0x8-0xF
        defb 0,0fah,0f8h,0f6h,0f4h,09,027h,0
;0x10-0x17
        defb 0,0e3h,0e1h,0,0e2h,'q','1',0
;0x18-0x1F
...
;0x50-0x57
        defb 0,0,027h,0,'[','=',0,0
;0x58-0x5F
        defb 0e4h,0e1h,0dh,']',0,05ch,0,0
;0x60-0x67
        defb 0,0,0,0,0,0,08h,0
;0x68-0x6F
        defb 0,'1',0,'4','7',0,0,0
;0x70-0x77
        defb '0','.','2','5','6','8',01bh,0e5h
;0x78-0x7F
        defb 0fbh,'+','3','-','*','9',0e6h,0
;;;;;;;;;;;Shifted scan code;;;;;;;;;;;;;;
;0x0-0x7
        defb 0,0f9h,0,0f5h,0f3h,0f1h,0f2h,0fch
;0x8-0xF
        defb 0,0fah,0f8h,0f6h,0f4h,09,'~',0
;0x10-0x17
        defb 0,0e3h,0e1h,0,0e2h,'Q','!',0
;0x18-0x1F
        defb 0,0,'Z','S','A','W','@',0
;0x20-0x27
        defb 0,'C','X','D','E','$','#',0
...
TCB_Dummy:	;TCB of dummy task - discarded after first StartUp call (10H here)
	...
	defs	25H	;this will be the impacted area of stack (25H here)
def_sp:			;16H here = TOTAL ~ 70H
	defs	2	;BC'
	defs	2	;DE'
	defs	2	;HL'
	defs	2	;AF'
...
IF	SYSSTS
	GLOBAL	CountTicks
CountTicks:defs 1	;0=do NOT count, 1=do count
TotalTicks:defs	4	;total ticks
ENDIF
...
_Signal:
	PUSH_REGS
	ld	hl,SigSem
	add	hl,sp		;stack=AF,BC,DE,HL,IX,IY,AF',BC',DE',HL',retaddr,Sem addr
	ld      a,(hl)
	inc	hl
	ld      h,(hl)
	ld	l,a		;HL=Sem list header pointer
...
1:				;yes : reschedule, CARRY=0
	...
	ld	hl,-18		;space for 9 push (HL,DE,BC,IX,IY,AF',BC',DE',HL')
	add	hl,sp
	ld	sp,hl
	jp	_Reschedule
;
...
;	on stack: BC',DE',HL',AF',IY,IX,BC,DE,HL,AF,Return address
;
...
;	on stack: AF',BC',DE',HL',AF,BC,DE,HL,IX,IY,Return address
;	Prepare current task stack to resume later,
;	then give control to first TCB from active tasks list
;
_ReschINT:
...
IF	PS2
        ...
        ld 	(fNumKey),a	;clear Num Keypad flag
        in 	a,(PS2Data)	;read clear keyboard buffer
	xor	a
ENDIF
IF	SYSSTS
	ld	(CountTicks),a	;disable sampling
ENDIF
	ld	l,a
	ld	h,a
	ld	(LastActiveTCB),hl
...
IF	SYSSTS
	ld	hl,0
	ld	(TCB_Default+TICS_OFF),hl
	ld	(TCB_Default+TICS_OFF+2),hl
ENDIF
	ld	hl,def_sp
	ld	(DefSP),hl
	ld	hl,def_sem
...
ENDIF
	...
	call	_InitBMem
IF	Z80ALL
	ld	hl,MoveBytes32K	;store at 0DEF6H the move bytes routine
	ld	de,MOVEBYTES32K
	ld	bc,0AH
	ldir
ENDIF
...
;	Move bytes to/from 32KB RAM bank - 10 bytes
;
;	A = BankNr ( 1 or 2 ), DE = dest, HL = source, BC = count
;
MoveBytes32K:
	di
	out	(B32K_PORT),a	;load desired bank
	ldir			;move bytes
	xor	a		;restore bank #0
	out	(B32K_PORT),a
	ei
	ret
;
...
_StartUp:
	PUSH_REGS
	ld	hl,OFF_TCB_SIZE
	add	hl,sp		;stack=AF,BC,DE,HL,IX,IY,AF',BC',DE',HL',retaddr,params
	ld	c,(hl)
	inc	hl
	ld	b,(hl)		;BC=TCBSize
	inc	hl
...
__Resume:
	...
	ld	hl,-20		;space for 10 push (HL,DE,BC,IX,IY,AF',BC',DE',HL')
	add	hl,sp
	ld	sp,hl
	jp	_Reschedule
1:
...
_Resume:
	PUSH_REGS
	ld	hl,ResTCB
	add	hl,sp		;stack=AF,BC,DE,HL,IX,IY,AF',BC',DE',HL',retaddr,TCB
	ld	a,(hl)
	inc	hl
	ld	h,(hl)
	ld	l,a		;HL=TCB
...
StopCheck:			;the current active task was stopped
	...
	jp	ShutDownLoop	;only the system tasks remain, go shutdown	
;
;	Quick Stop Task
;
...
IF	SYSSTS
	push	hl
	call	SaveTCBinfo	;save some data to VGA tcb table
	pop	hl
ENDIF
...
;
;	Save some data from TCB to the VGA TabTCB record
;
;	HL = TCB
;
SaveTCBinfo:
	ld	de,TabTCB	;DE=TabTCB pointer
loopcmp:
	ld	a,(de)
	cp	l
	jr	nz,nexttcb
	...
	jr	nz,nexttcb
				;DE=TCB record found
	inc	de
	inc	de		;DE=cursor pointer
	push	hl		;TCB on stack
	ld	bc,CURSOR_OFF
	add	hl,bc
	ld	a,(hl)		;save VGA cursor
	ld	(de),a
	inc	hl
	inc	de
	ld	a,(hl)
	...
	inc	hl
	inc	de
	pop	hl		;HL=TCB,DE=free stack pointer
	ex	de,hl		;DE=TCB,HL=free stack pointer
	push	hl		;free stack pointer on stack
	call	__StackLeft	;HL=free stack size
	pop	de		;DE=free stack pointer on stack
	ex	de,hl		;DE=free stack size, HL=free stack pointer on stack
	ld	(hl),e		;save free stack size
	inc	hl
	ld	(hl),d
	ret
nexttcb:
	ex	de,hl		;HL=TabTCB pointer, DE=TCB
	ld	bc,TABTCB_SIZE
	add	hl,bc
	ex	de,hl		;DE=TabTCB pointer, HL=TCB
	jr	loopcmp
...
IF	SYSSTS
	...
	ld	a,(TaskCnt)	;A=tasks counter
	push	af		;on stack
				;compute Remaining stack size for each task
	ld	hl,TabTCB
	ld	(TabTCB_P),hl
stack_size:
	ld	hl,(TabTCB_P)
	...
	call	IsItTask
	jr	z,1f
				;task was stopped, fetch data from TabTCB
	ld	hl,(TabTCB_P)
	inc	hl
	inc	hl		;HL=cursor pointer
	ld	c,(hl)
	inc	hl
	ld	b,(hl)		;BC=cursor
	inc	hl
	...
	inc	hl
	inc	hl
	inc	hl		;HL=free stack size pointer
	ld	a,(hl)
	inc	hl
	ld	h,(hl)
	ld	l,a		;HL=free stack size
	jr	2f
1:				;task was not stopped 
	ld	de,STACKW_OFF+1
	add	hl,de
	ld	d,h
	ld	e,l		;HL=DE=TCB+STACKW_OFF+1
lfnz:
	ld	c,(hl)
	inc	hl
	ld	a,(hl)
	...
	or	c
	jr	z,lfnz
	sbc	hl,de		;HL=remaining stack size
	ex	de,hl		;HL=TCB+STACKW_OFF+1, DE=remaining stack size
	ld	bc,STACKW_OFF-CURSOR_OFF
	sbc	hl,bc		;HL=TCB+CURSOR_OFF+1
	ld	b,(hl)
	dec	hl
	ld	c,(hl)		;BC=cursor
	ex	de,hl		;HL=remaining stack size
2:
	call	TypeRemStack

	ld	hl,(TabTCB_P)	;move tcb pointer to the next record
	ld	bc,TABTCB_SIZE
	add	hl,bc
	ld	(TabTCB_P),hl
...
3:
	ld	c,(hl)
	inc	hl
	ld	b,(hl)		;BC=(low)task ticks
	inc	hl
	ld	e,(hl)
	inc	hl
	ld	d,(hl)		;DE=(high)task ticks
	ld	hl,(TotalTicks)
	add	hl,bc
	ld	(TotalTicks),hl
	ld	hl,(TotalTicks+2)
	...
	ld	(TaskCnt),a
				;TotalTicks computed
				;now compute (task ticks * 100)/TotalTicks
	xor	a		;use Sum@ as the sum of %
	ld	(Sum@),a
...
3:
	...
	inc	hl
	ld	h,(hl)
	ld	l,a		;HL DE=task tics
	ld	bc,0		;compute task ticks x 100
	push	bc
	ld	bc,100
	push	bc
	call	mul32		;task ticks x 100 = HL DE = HL DE x 0 100 , 0 100 removed from stack 
	push	de
	exx
	pop	hl
	exx			;HL = high(task ticks x 100), HL' = low(task ticks x 100)
	ld	bc,(TotalTicks)
	push	bc
	exx
	pop	de
	exx
	ld	de,(TotalTicks+2);DE = high(total ticks), DE' = low(total ticks)
	call	div32		;BC BC' = HL HL' / DE DE', remainder HL HL'
				;see adjustment
				;((HL, HL') x 2) ? TotalTicks
	ld	b,0		;init increment = 0
...
addb:
	exx
	ld	a,c
	exx			;A=(task ticks x 100) / TotalTicks
	add	a,b		;adjust
	ld	e,a		;E=(task ticks x 100) / TotalTicks
	...
	jr	z,donecheck
				;sum % is not 100
	jr	nc,toobig
				;too small, need to adjust last %
	neg
	add	a,e		;add to last %
	ld	e,a
	jr	donecheck
toobig:				;too big, need to adjust last %
	ld	d,a		;subtract from last %
	ld	a,e
	or	a
	jr	z,donecheck
	sub	d
	ld	e,a
donecheck:			;type E=% in decimal at cursor
	pop	bc		;BC=cursor
