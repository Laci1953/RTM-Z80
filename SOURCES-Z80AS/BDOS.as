...
BSC:DEFW E1;bad sector on read or write.
BSL:DEFW E2;bad disk select.
ROD:DEFW E3;disk is read only.
ROF:DEFW E4;file is read only.
...
E1:LD HL,BADSEC;bad sector message.
	RET
E2:LD HL,BADSEL;bad drive selected.
	RET
E3:LD HL,DISKRO;disk is read only.
	RET
E4:LD HL,FILERO;file is read only.
	RET
SETS:LD (STS),A
RTN:RET 
IOE1:LD A,1
...
STB:PUSH BC;save 16 bit word.
	...
	LD C,A
	LD HL,1
	CALL SL;shift bit 0 into place.
...
WPD:LD HL,WP;point to status word.
	...
	RET 
CKF:CALL FHL;set (HL) to file entry in directory buffer.
...
CKW:CALL GWP
	...
	JP JHL
FHL:LD HL,(DB);get address of buffer.
...
NXEY:LD HL,(DIS);get directory entry size limit.
	...
	LD (FP),HL
	CALL SHL;(HL)=(DIS)-(FP)
	JP NC,1f;is there more room left?
	JP STFP;no. Set this flag and return.
1:LD A,(FP);get file position within directory.
...
STB1:RRCA;restore original bit position.
	DEC D
	JP NZ,STB1
	LD (HL),A;and stor byte in table.
	RET 
STFL:CALL FHL;get address of fcb
	LD DE,16
...
1:CALL FHL;get address of this fcb in directory.
...
1:CALL CKFP;any found?
	...
	CALL FHL;nope, get address of fcb and
...
UPD:PUSH DE
	...
	EX DE,HL
	CALL FHL;get address of fcb to update in directory.
...
1:CALL CKFP;any matching file found?
	...
	JP 1b;and continue.
STR:LD C,12;match first 12 bytes.
...
OPN1:CALL STE;point to extent byte within users fcb.
	...
	PUSH HL
	CALL FHL;point to fcb in directory.
	EX DE,HL
...
1:LD (HL),A
	...
	CALL FBS;write out this fcb entry to directory.
...
4:CALL STD;move in updated data (rec #, extent #, etc.)
	...
	JP SETS
5:CALL IOE1;set error code, clear bit 7 of 's2'
...
1:CALL CKFP;is there an extent to process?
	...
	CALL FHL;set (HL) pointing to proper fcb in dir.
